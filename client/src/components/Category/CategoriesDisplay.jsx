...
function CategoriesDisplay() {
    ...
    {[1, 2, 3, 4, 5, 6, 7, 8].map((item) => (
        <div key={item} className='group flex flex-col items-center cursor-pointer'>
            {/* The Border Container - Forced to be a circle */}
            <div className='relative overflow-hidden rounded-full p-1 border-2 border-transparent group-hover:border-pink-200 transition-all duration-500 w-fit h-fit'>
                <img 
                    src={Woman} 
                    alt="Woman"
                    /* h-20 w-20 on mobile, h-28 w-28 on md+ screens for a perfect 1:1 circle */
                    className='w-40 h-40 md:w-28 md:h-28 rounded-full object-cover transition-transform duration-500 group-hover:scale-110'
                />
            </div>
            {/* Text Label */}
            <span className='mt-3 font-medium text-gray-600 text-sm tracking-wide group-hover:text-pink-500 transition-colors'>
                Woman
            </span>
        </div>
    ))}
...
<div className='w-full space-y-8'>
    ...
    <div className='group relative overflow-hidden  rounded-sm py-2 px-4 cursor-pointer text-center shadow-lg border-b-2 border-pink-400 '>
        {/* Animated Gradient Overlay */}
        <div className='absolute inset-0  opacity-0 group-hover:opacity-100 transition-opacity duration-700'></div>
        
        {/* Banner Text */}
        <span className='relative font-bold text-2xl tracking-[0.3em] text-pink-500 block transform group-hover:scale-105 transition-transform duration-500'>
            DAILY FINDS
        </span>
    </div>
